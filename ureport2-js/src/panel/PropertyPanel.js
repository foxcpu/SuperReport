...
import DoubleValueEditor from './property/DoubleValueEditor.js'
...
export default class PropertyPanel{
    ...
    buildPanel(){
        ...
        const doubleValueEditor=new DoubleValueEditor(this.panel,this.context);
        this.editorMap.set('number',doubleValueEditor);
    ...
    _buildNeedFill(){
        this.needFillGroup=$(`<div class="form-group" style="margin-bottom:10px;margin-top: 10px;"><label>${window.i18n.property.prop.cellType}</label></div>`);
    }
    ...
    _buildCellType(){
        ...
        this.typeSelect=$(`<select class="form-control" style="display: inline-block;width:280px;padding: 3px;font-size: 12px;height: 25px;">
            ...
            <option value="number">${window.i18n.property.prop.number}</option>
        </select>`);
        ...
        this.typeSelect.change(function(){
            ...
            if(value==='simple'){
                ...
                if(cellDef.value.type!=='number'){
                    cellDef.value={type:'number'};
                }
                cellDef.expand='None';
                _this.editorMap.get('number').show(_this.cellDef,_this.rowIndex,_this.colIndex,_this.row2Index,_this.col2Index);
